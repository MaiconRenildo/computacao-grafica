...
void drawCar(){
    glPushMatrix();
        // std::cout << carPosition << std::endl;
        if (carPosition >= 12.51) {
            carPosition = -0.5;
        } else if (carPosition <= -0.51) {
            carPosition = 12.5;
        }
        glTranslated((carPosition), 0.0, 0.0);
        glPushMatrix();
...
void drawWindTower(float rotate=10.0){
    // pás
    glPushMatrix();
        glRotated(frameNumber/0.1, 0.0, 0.0, 1.0);
        drawBlades();
    glPopMatrix();
...
void drawBlueCircle(){
    ...
    int aleatorioy = std::rand() % 2; // número entre 0 e 1
    float y = 0;
    if(aleatorioy==1){
        y = 0.65;
    }
    float x = -4.5f + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX) / 9.0f);  // número entre -4.5 e 4.5
    glPushMatrix();
...
// funciona para teclas comuns
void keyboard( unsigned char key, int x, int y )
{
    // std::cout << key << std::endl;
	switch( key ) {
	case 'f' : case 'F' :
		carPosition = carPosition+ 0.5;
	break;
...
// funciona para teclas especiais
